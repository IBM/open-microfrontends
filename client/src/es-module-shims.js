  ...
  const featureDetectionPromise = Promise.all([
    ...
    supportsDynamicImport && hasDocument && new Promise(resolve => {
      self._$s = v => {
        ...
        if (v) supportsImportMaps = false; //CUSTOM PATCH: patched for supporting shim in chrome (normally this variable will resolved to true here) But we want to use the shimed version to have support for multiple import maps.
        delete self._$s;
        resolve();
      };
